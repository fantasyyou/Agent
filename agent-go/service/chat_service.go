...
func NewChatService(memory MemoryRepository, sessions SessionRepository, usage UsageRepository, agent AgentClient, limit int) *ChatService {
	return &ChatService{memory: memory, sessions: sessions, usage: usage, agent: agent, limit: limit}
}
func (s *ChatService) Ask(ctx context.Context, userID, sessionID, question string) (string, error) {
	...
	response, err := s.agent.Answer(ctx, model.AgentRequest{RequestID: requestID, UserID: userID, SessionID: sessionID, Question: question, Memories: memories})
	if err != nil {
